...
contract TheLPTest is Test {
    ...
    //     lp.mint{value: 3.33 ether * 10}(10);
    //     for (uint256 i = 0; i < 10; i++) {
    //         console2.log(lp.tokenURI(i + 1));
    //     }
    //     assertEq(true, true);
    // }
    ...
    function testMintShouldRevertIfTryingToTransferPriorToEnd() public {
        address user = address(1);
        vm.deal(user, 1000 ether);
        vm.startPrank(user);
        uint256 price = lp.getCurrentMintPrice();
        lp.mint{value: price}(1);
        uint256[] memory tokens = lp.tokensOfOwner(user);
        vm.expectRevert(TheLP.NotLockedIn.selector);
        lp.transferFrom(user, address(2), tokens[0]);
        vm.stopPrank();
    }
    ...
    function testMint_should_revert_if_amount_0() public {
        vm.expectRevert(TheLP.AmountRequired.selector);
        lp.mint{value: 0}(0);
    }

    function testMint_should_revert_if_value_0() public {
        vm.expectRevert(TheLP.IncorrectPayment.selector);
        lp.mint{value: 0}(1);
    }

    function testMint_should_start_at_tokenID_1001() public {
        uint256 price = lp.getCurrentMintPrice();
        lp.mint{value: price}(1);
        address owner = lp.ownerOf(1001);
        uint256 totalSupply = lp.totalSupply();
        assertEq(owner, address(this));
        assertEq(totalSupply, 1001);
    }
    ...
    function testMint_should_save_token_data_for_range_of_tokens() public {
        lp.mint{value: 9.99 ether}(3);

        for (uint256 i = 1000; i < 1003; i++) {
            (, uint256 cost) = lp.tokenMintInfo(i + 1);
            assertEq(cost, 1.11 ether);
        }
    }

    function testMint_should_refund_additional_amount() public {
        uint256 balanceBeforeMint = address(this).balance;
        lp.mint{value: 4 ether}(1);
        uint256 balanceAfterMint = address(this).balance;

        assertEq(balanceBeforeMint - balanceAfterMint, 1.11 ether);
    }

    function testMint_should_lock_it_in_and_transfer_funds() public {
        for (uint256 i = 0; i < 32; i++) {
            if (i == 31) {
                // This contract is the owner.
                // When locked in is called this contract should receive half of proceeds

                vm.expectEmit(false, false, false, true);
                emit PaymentReceived(address(lp), (1.11 ether * 8000) / 2);
            }
            lp.mint{value: 1.11 ether * 250}(250);
        }
    ...
    function testGetCurrentPrice_ShouldDecreaseOverTime() public {
        for (uint256 i = 1; i <= 12; i++) {
            ...
            if (i == 0) {
                assertEq(currentMintPrice, 1.11 ether);
            }
            if (i == 12) {
        ...
        uint256 currentMintPrice = lp.getCurrentMintPrice();
        console2.log(currentMintPrice);
        assertEq(currentMintPrice, 11138156250345588);
    }
    ...
    function test_redeem_ShouldRedeemForMultipleTokens() public {
        ...
        lp.mint{value: 1.11 ether * 5}(5);
        uint256 balanceAfterMint = address(testAddress).balance;
        assertEq(1.11 ether * 5, balanceBeforeMint - balanceAfterMint);
    ...
    function test_redeem_ShouldRedeemForSingleToken() public {
        ...
        vm.startPrank(testAddress);
        uint256 balanceBeforeMint = address(testAddress).balance;
        lp.mint{value: 1.11 ether}(1);
        uint256 balanceAfterMint = address(testAddress).balance;
        assertEq(1.11 ether, balanceBeforeMint - balanceAfterMint);
        vm.warp(block.timestamp + 35 days);
    ...
    function test_redeem_ShouldThrowIfTryingToRedeemTwice() public {
        ...
        lp.redeem(tokens);
        vm.expectRevert(
            abi.encodePacked(TheLP.InvalidTokenId.selector, uint256(tokenId))
        );
        lp.redeem(tokens);
    }
    ...
    function test_redeem_ShouldThrowIfNotTokenOwnerRedeeming() public {
        ...
        vm.expectRevert(
            abi.encodePacked(TheLP.NotOwner.selector, uint256(tokenId))
        );
        lp.redeem(tokens);
    }
    ...
    function test_buy_shouldBuyForSaleToken() public {
        ...
        uint256 prevFeeBalance = lp.getFeeBalance();
        ...
        assertEq(feeBalance - prevFeeBalance, fee / 2);
        vm.stopPrank();
    ...
    function test_buy_ShouldBeAbleToBuyInstantMintedNFTsAfterSellout() public {
        _lockItIn();
    ...
    function test_sell_ShouldDecreaseBuyPriceEachTime() public {
        _lockItIn();
        uint256[] memory tokens = lp.tokensOfOwner(address(lp));
        uint256[] memory owned = lp.tokensOfOwner(address(this));
        for (uint256 i = 0; i < owned.length; i++) {
            (uint256 sellPrice, uint256 fee) = lp.getSellPrice();
            // console2.log(sellPrice);
    ...
    function test_sell_shouldWork() public {
        _lockItIn();
    ...
    function test_sell_should_sell_multiple() public {
        ...
        uint256 afterBalance = address(this).balance;
        assertEq(afterBalance - balanceBefore, out);
    }
    ...
    function test_sell_shouldSellMultipleFromDifferentWallets() public {
        ...
        vm.startPrank(testAddress);
        (uint256 sellPrice1, ) = lp.getSellPrice();
        lp.sell(7);
        (uint256 sellPrice2, ) = lp.getSellPrice();
        lp.sell(10);
        (uint256 sellPrice3, ) = lp.getSellPrice();

        lp.sell(4532);
        afterBalance = address(testAddress).balance;
        assertEq(
            afterBalance - balanceBefore,
            sellPrice1 + sellPrice2 + sellPrice3
        );
        vm.stopPrank();

        uint256[] memory tokensForSale = lp.tokensOfOwner(address(lp));
    ...
    function lockItInVars(uint256 loops, uint256 amount) internal {
        for (uint256 i = 0; i < loops; i++) {
            lp.mint{value: 3.33 ether * amount}(amount);
        }
    }
